...
.EXAMPLE
  ...
  .\scripts\gce-sync.ps1 -WithL2    # Mode B L2 netns smoke (implies CWL pack + link)
#>
param(
...
if ($WithL2) { $WithCwl = $true }
if ($WithCwl -and -not $cwlPresent) {
  throw "-WithCwl/-WithL2 requires sibling chrysalis-cwl at $cwlRootSibling"
}
